...
<p align="center">
  ...
  <img src="https://img.shields.io/github/forks/bohd4nx/TGgifts-buyer" alt="Форки">
  <img src="https://img.shields.io/github/issues/bohd4nx/TGgifts-buyer" alt="Проблемы">
  <br>
  <img src="https://img.shields.io/badge/разработчик-@bohd4nx-blue.svg" alt="Telegram">
...
Бот отправляет подарки конкретным пользователям и может быть настроен для работы как с лимитированными, так и с обычными
подарками.
...
- **Описание**: Ваши API ID и HASH для Telegram. Чтобы получить эти данные, необходимо создать приложение
  на [сайте Telegram](https://my.telegram.org/auth).
...
- **Описание**: Список ID пользователей, которым бот будет отправлять подарки. Пользователи должны быть в контактах с
  ботом (т.е. бот и пользователь должны быть в одной группе или канале).

### `MIN_GIFT_PRICE`

- **Описание**: Минимальная допустимая цена подарка. Бот будет отправлять только подарки, стоимость которых выше этого
  лимита.
- **По умолчанию**: `0`

### `MAX_GIFT_PRICE`

- **Описание**: Максимально допустимая цена подарка. Бот будет отправлять только подарки, стоимость которых ниже этого
  лимита.
- **По умолчанию**: `10000`

### `PURCHASE_NON_LIMITED_GIFTS`

- **Описание**: Если `True`, бот будет покупать неограниченные подарки, как только они появятся, при этом соблюдая лимит
  цены подарка.
- **По умолчанию**: `False`
...
- **Описание**: Задержка (в секундах) между последовательными покупками подарков одному или разным пользователям.
- **По умолчанию**: `5` _Рекомендуется для избежания ограничений и обнаружения._

## Пользовательские настройки

### Внесение корректировок

- Вы можете изменять значения параметров `USER_ID`, `MAX_GIFT_PRICE`, `MIN_GIFT_PRICE`, `PURCHASE_NON_LIMITED_GIFTS` и т.д прямо в файле
  конфигурации, чтобы настроить, кому и какие подарки будет отправлять бот.
- Установите `PURCHASE_NON_LIMITED_GIFTS` в `False`, если не хотите, чтобы бот автоматически покупал неограниченные
  подарки.

### Поддерживаемые языки

Бот поддерживает несколько языков, чтобы сделать его доступным для пользователей по всему миру. Локализация позволяет
отправлять уведомления и сообщения на предпочтительном языке пользователя.
...
Локализация управляется через языковые файлы, расположенные в директории `locales/`. Чтобы изменить язык бота, нужно
изменить настройку `LANGUAGE` в конфигурации.
...
Вот базовая структура языкового файла (например, [
...
Бот начнёт отправлять подарки в соответствии с настройками. Он будет проверять новые подарки через заданный интервал (
`INTERVAL`) и отправлять их указанным пользователям. Уведомления будут отправляться в Telegram канал, указанный в
`CHANNEL_ID`.
...
- **Описание**: Бот сталкивается с ошибкой `AttributeError`, которая указывает на отсутствие метода `get_star_gifts` в
  объекте `Client`.
- **Инфо**: Эта ошибка возникает, когда установленная версия библиотеки `pyrogram` не включает метод `get_star_gifts`,
  что может быть связано с устаревшей или некорректной версией библиотеки.
...
Этот бот создан исключительно в образовательных целях. Используйте его на свой страх и риск. Разработчик не несет
ответственности за любое неправильное использование или ущерб, причиненный этой программой.
